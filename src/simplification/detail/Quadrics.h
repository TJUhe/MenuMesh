...
/// Evaluates homogeneous error `[p,1]^T q [p,1]`.
...
/// Initial per-vertex quadrics plus queue-priority factors decoupled from them.
///
...
/// Accumulates all enabled initial quadric terms and report diagnostics.
...
/// Returns unique finite placement candidates sorted by ascending quadric cost.
...
/// Builds all initial geometry and guidance quadrics for one immutable input mesh.
