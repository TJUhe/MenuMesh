...
std::vector<PlaneCluster> clusterCoplanarFaces(const Mesh& mesh, double normalTolerance, double offsetTolerance) {
    ...
    for (const Face& face : mesh.faces) {
        ...
        if (normal.z() < 0.0 || (std::abs(normal.z()) <= 1e-12 &&
                                 (normal.y() < 0.0 || (std::abs(normal.y()) <= 1e-12 && normal.x() < 0.0)))) {
            normal = -normal;
            offset = -offset;
        }

        auto it = std::find_if(clusters.begin(), clusters.end(), [&](const PlaneCluster& cluster) {
...
bool hasPlaneCluster(const std::vector<PlaneCluster>& planes, const Vec3& normal, double offset, double minArea) {
    ...
    return std::any_of(planes.begin(), planes.end(), [&](const PlaneCluster& plane) {
        return plane.normal.dot(n) > 1.0 - 1e-12 && std::abs(plane.offset - offset) < 1e-10 && plane.area > minArea;
    });
}
...
Mesh makeBranchedCircularBoundaryMesh() {
    ...
    for (int i = 0; i < kSegments; ++i) {
        const double angle = 2.0 * pi * static_cast<double>(i) / static_cast<double>(kSegments);
...
Mesh makeFragmentedCircleWithTensorRidgeMesh() {
    ...
    for (const Face& face : ridge.faces) {
        mesh.faces.push_back({{face.v[0] + offset, face.v[1] + offset, face.v[2] + offset}});
    }
    return mesh;
}
...
bool hasClosedLoopWithVertices(const FeatureAnalysis& features, const std::vector<int>& expectedVertices) {
    ...
    return std::any_of(features.loops.begin(), features.loops.end(), [&](const FeatureLoop& loop) {
