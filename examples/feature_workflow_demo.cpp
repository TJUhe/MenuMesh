...
QualityGateResult runManufacturingQualityGate(const manumesh::Mesh& input,
                                              const QualityGateOptions& gateOptions) {
  if (input.empty()) {
  ...
  const manumesh::Mesh output = manumesh::simplification::QEMSimplifier(simplifyOptions)
                                    .simplify(input, &simplifyReport);
  ...
  const manumesh::simplification::DistanceStats distance =
      manumesh::simplification::compareMeshesBySampledDistance(
          input, output, gateOptions.distanceSamples);
  ...
  result.report.maxSampledDistance =
      std::max(distance.maxOriginalToSimplified, distance.maxSimplifiedToOriginal);

  result.accepted = !output.empty() && outputStats.faces < inputStats.faces &&
                    outputStats.area > 0.0 &&
...
int main() {
  ...
  std::cout << "feature_workflow_demo"
            << " input_faces=" << result.report.inputFaces
            << " output_faces=" << result.report.outputFaces
