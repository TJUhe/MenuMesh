...
```powershell
# MeshCore Debug：保留 STL/OBJ、网格分析、特征检测和 QEM 简化
cmake --preset vs2019-debug-meshcore
cmake --build --preset vs2019-debug-meshcore
...
`vs2019-debug-meshcore` 是另一个固定的 MeshCore 库工作区，不会改变上面的完整 Debug 配置。解决方案只
包含 `Core`、`Common`、`MeshEdit`、`IO`、`Analysis`、`FeatureDetection`、`Simplification` 和
`ALL_BUILD`。只有 `Core` 是共享库，输出
`build/vs2019-debug-meshcore/bin/Debug/MeshCore.dll` 与
`build/vs2019-debug-meshcore/lib/Debug/MeshCore.lib`；其他功能项目是用于源码浏览的 CMake `OBJECT`
目标，其对象文件由 `Core` 汇总，不作为独立交付库。该版本保留普通 STL/OBJ 读写、网格分析、
特征检测和特征保护 QEM 简化；不包含 CLI、测试、示例、C API、MMPD 超大网格、纹理坐标简化、
oneTBB 或第三方 Visual Studio 项目。它关闭 CMake 自动重生成，因此修改 `CMakeLists.txt` 或增删
MeshCore 源文件后，需要重新执行一次 `cmake --preset vs2019-debug-meshcore`。详细范围与构建说明见
[`documentation/guide/meshcore-workspace.md`](documentation/guide/meshcore-workspace.md)。
