...
double vertexScale(const std::vector<double>& scales, int vertex, double fallback) {
  if (vertex >= 0 && vertex < static_cast<int>(scales.size()) &&
      std::isfinite(scales[vertex]) && scales[vertex] > 1e-12) {
...
bool isWeakCleanupSpurEdge(const TraceGraph& trace, int a, int b) {
  return traceEdgeNormalTensor(trace, a, b) && !traceEdgeBoundary(trace, a, b) &&
         !traceEdgeDihedral(trace, a, b) && !traceEdgeNonManifold(trace, a, b) &&
         !traceEdgeCleanupBridge(trace, a, b);
}
...
void removeWeakSpurs(const FeatureOptions& options, TraceGraph& trace,
...
collectEndpoints(const Mesh& mesh, const TraceGraph& trace,
  ...
  for (int id = 0; id < static_cast<int>(trace.adjacency.size()); ++id) {
    ...
    outward.normalize();
    endpoints.push_back(
        EndpointCandidate{id, nb, outward, vertexScale(localScale, id, fallbackScale)});
  }
  return endpoints;
}
...
void bridgeEndpointGaps(const Mesh& mesh, const FeatureOptions& options,
  ...
  const double fallbackScale =
      meanPositiveScale(localScale, std::max(1e-12, mesh.bboxDiag() * 1e-3));
  const std::vector<EndpointCandidate> endpoints =
      collectEndpoints(mesh, trace, localScale, fallbackScale);
...
void bridgeCloseJunctions(const Mesh& mesh, const FeatureOptions& options,
                          TraceGraph& trace, FeatureAnalysis& analysis) {
  if (options.featureGraphGapLengthRatio <= 0.0) {
    return;
  }
...
double primitiveResidual(const Mesh& mesh, const FeatureLoop& loop) {
  ...
  case FeaturePrimitiveType::Ellipse: {
    const double scale =
        std::max({loop.majorRadius, loop.minorRadius, diag * 1e-9, 1e-12});
...
double computeConfidence(const FeatureComponent& component,
  ...
  const double residualScore =
      hasPrimitiveResidual
          ? std::clamp(1.0 - component.meanPrimitiveResidual / 0.12, 0.0, 1.0)
          : 0.5;
  const double junctionPenalty =
      component.junctionVertices > 2
...
int dominantComponentForLoop(const FeatureLoop& loop,
...
void summarizeFeatureComponents(const Mesh& mesh, const FeatureOptions& options,
  ...
  for (int seed = 0; seed < static_cast<int>(trace.adjacency.size()); ++seed) {
    ...
    while (!queue.empty()) {
      ...
      for (int nb : trace.adjacency[v]) {
        if (v < nb) {
          ++component.edgeCount;
          if (traceEdgeBoundary(trace, v, nb)) ++component.boundaryEdges;
          if (traceEdgeDihedral(trace, v, nb)) ++component.dihedralEdges;
          if (traceEdgeNormalTensor(trace, v, nb)) ++component.normalTensorEdges;
          if (traceEdgeNonManifold(trace, v, nb)) ++component.nonManifoldEdges;
          if (traceEdgeCleanupBridge(trace, v, nb)) ++component.cleanupBridgeEdges;
        }
        if (!visited[nb]) {
          visited[nb] = 1;
          queue.push(nb);
    ...
    component.cycleRank =
        component.edgeCount - static_cast<int>(component.vertices.size()) + 1;
    component.closed = component.endpointVertices == 0 && component.edgeCount > 0 &&
                       component.cycleRank >= 0;
  ...
  for (FeatureLoop& loop : analysis.loops) {
    const int componentId = dominantComponentForLoop(
        loop, vertexToComponent, static_cast<int>(analysis.components.size()));
  ...
  if (!analysis.components.empty()) {
    analysis.meanFeatureComponentConfidence =
        confidenceSum / static_cast<double>(analysis.components.size());
  ...
  for (FeatureLoop& loop : analysis.loops) {
    if (loop.componentId >= 0 &&
        loop.componentId < static_cast<int>(analysis.components.size())) {
  ...
  for (int id = 0; id < static_cast<int>(analysis.vertices.size()) &&
                   id < static_cast<int>(vertexToComponent.size());
       ++id) {
    const int componentId = vertexToComponent[id];
    if (componentId < 0 ||
        componentId >= static_cast<int>(analysis.components.size())) {
      continue;
    }
