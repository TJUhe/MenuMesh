...
Mesh generateLatheProfile(const std::vector<std::pair<double, double>>& rz, int segments) {
    ...
    for (int j = 0; j + 1 < static_cast<int>(rz.size()); ++j) {
        for (int i = 0; i < segments; ++i) {
            ...
            addQuad(mesh, index[j][i], index[j][next], index[j + 1][next], index[j + 1][i]);
        }
    }
    ...
    for (int y = 0; y <= n; ++y) {
        for (int x = 0; x <= n; ++x) {
            mesh.vertices.emplace_back(
                coordFromIndex(x, n, size, clustered), coordFromIndex(y, n, size, clustered), 0.0
            );
        }
    }
...
Mesh generateHolePlaneGrid(int n, double size, double radius) {
    ...
    for (int y = 0; y <= n; ++y) {
        for (int x = 0; x <= n; ++x) {
            mesh.vertices.emplace_back(
                (static_cast<double>(x) / n - 0.5) * size, (static_cast<double>(y) / n - 0.5) * size, 0.0
            );
        }
    }
...
Mesh generateBumpGrid(int n, double size) {
    ...
    for (int y = 0; y <= n; ++y) {
        for (int x = 0; x <= n; ++x) {
            ...
            const double cross = 0.08 * std::exp(-180.0 * px * px) + 0.08 * std::exp(-180.0 * py * py);
...
Mesh generateCylinderGrid(int radialSegments, int heightSegments, double radius, double height) {
    ...
    for (int i = 0; i < radialSegments; ++i) {
        ...
        mesh.faces.push_back({{topCenter, heightSegments * radialSegments + i, heightSegments * radialSegments + j}});
    }
    return mesh;
}
...
Mesh generateCubeGrid(int n, double size) {
    ...
    appendFaceGrid(mesh, GridPatch{n, size, Vec3(0, 0, half), Vec3(1, 0, 0), Vec3(0, 1, 0)});
    appendFaceGrid(mesh, GridPatch{n, size, Vec3(0, 0, -half), Vec3(1, 0, 0), Vec3(0, -1, 0)});
...
Mesh generateThinFinGrid(int n, double size) {
    ...
    appendRectGrid(
        mesh,
        RectGridPatch{nx, ny, Vec3(0.0, 0.0, 0.5 * finHeight), Vec3(0.0, finLength, 0.0), Vec3(0.0, 0.0, finHeight)}
    );
    appendRectGrid(
        mesh,
        RectGridPatch{
            nx, ny, Vec3(finThickness, 0.0, 0.5 * finHeight), Vec3(0.0, finLength, 0.0), Vec3(0.0, 0.0, finHeight), true
        }
    );
    appendRectGrid(
        mesh,
    ...
    appendRectGrid(
        mesh,
        RectGridPatch{
            nt, nx, Vec3(0.5 * finThickness, 0.0, finHeight), Vec3(finThickness, 0.0, 0.0), Vec3(0.0, finLength, 0.0)
        }
    );
    return mesh;
}
