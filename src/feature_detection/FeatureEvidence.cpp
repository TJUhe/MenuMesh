...
struct EdgeEvidenceContext {
  EdgeEvidenceContext(const Mesh& inputMesh, const FeatureOptions& inputOptions,
                      FeatureAnalysis& outputAnalysis)
      : mesh(inputMesh), options(inputOptions), analysis(outputAnalysis),
...
int signedDihedralKind(const Mesh& mesh, const std::vector<Vec3>& normals,
                       const manumesh::detail::MeshEdgeInfo& info, int a, int b) {
...
public:
  ...
  virtual void classify(CandidateEdge& edge, const manumesh::detail::MeshEdgeInfo& info,
                        EdgeEvidenceContext& context) const = 0;
};

class BoundaryEvidenceStrategy final : public EdgeEvidenceStrategy {
public:
  void classify(CandidateEdge& edge, const manumesh::detail::MeshEdgeInfo& info,
                EdgeEvidenceContext&) const override {
...
public:
  void classify(CandidateEdge& edge, const manumesh::detail::MeshEdgeInfo& info,
                EdgeEvidenceContext& context) const override {
    if (info.faces.size() != 2) {
      return;
    }

    const double dot = std::clamp(
        std::abs(context.normals[info.faces[0]].dot(context.normals[info.faces[1]])),
        -1.0, 1.0);
    edge.angleRad = std::acos(dot);
    edge.dihedral = edge.angleRad >= context.dihedralThreshold;
    if (edge.dihedral) {
      edge.signedKind =
          signedDihedralKind(context.mesh, context.normals, info, edge.a, edge.b);
    }
  }
};
...
public:
  void classify(CandidateEdge& edge, const manumesh::detail::MeshEdgeInfo&,
                EdgeEvidenceContext& context) const override {
    if (edge.boundary || edge.dihedral || edge.nonManifold) {
      return;
    }
    edge.normalTensor =
        normalTensorEdgeCandidate(edge, context.tensor, context.discreteFeatureVertex,
                                  context.mesh, context.options, context.analysis);
  }
};
...
std::vector<CandidateEdge> collectFeatureEdges(const Mesh& mesh,
  ...
  const std::array<const EdgeEvidenceStrategy*, 4> strategies = {
      &boundaryEvidence, &dihedralEvidence, &nonManifoldEvidence,
      &normalTensorEvidence};
