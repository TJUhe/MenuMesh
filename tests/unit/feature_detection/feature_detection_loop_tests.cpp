...
TEST(FeatureDetection, DetectsCircularCylinderBoundaryLoops) {
    ...
    const auto loopIt = std::find_if(features.loops.begin(), features.loops.end(), [](const FeatureLoop& loop) {
...
TEST(FeatureDetection, RecoversCircularLoopFromBranchedFeatureGraph) {
    ...
    const std::vector<FeatureLoop> unitLoops = circularLoopsNearRadius(features, 1.0, 1e-8);
