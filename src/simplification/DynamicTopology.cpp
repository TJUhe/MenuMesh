...
std::vector<std::pair<int, int>> collectActiveEdges(const std::vector<FaceState>& faces) {
...
bool areAdjacent(int a, int b, const std::vector<FaceState>& faces, const DynamicTopology& topology) {
...
int activeIncidentFaceCountForEdge(int a, int b, const std::vector<FaceState>& faces, const DynamicTopology& topology) {
    if (a < 0 || b < 0 || a >= static_cast<int>(topology.vertexFaces.size()) ||
        b >= static_cast<int>(topology.vertexFaces.size())) {
        return 0;
    }
...
BoundaryCollapseDecision boundaryCollapseDecision(const BoundaryCollapseInput& input) {
    ...
    const bool isCurrentBoundaryEdge = activeIncidentFaceCountForEdge(keep, remove, faces, topology) == 1;
