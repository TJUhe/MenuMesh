...
1. `EdgeEvidenceStage`：收集四类原始 evidence；需要法向时从 `FeatureDetectionCache` 取得原始或过滤后的面法向。
...
| `FeatureGraphCompatibility.cpp` | cleanup/consolidation 共用的兼容规则 | 检查方向、强/弱 evidence source 和 signed kind |
...
## 5. 图兼容、consolidation 与 junction

cleanup 的 endpoint gap 和 close-junction bridge 现在共享兼容规则：

- 连接方向必须延续两端已有分支。
- 两侧已知 signed kind 相反时拒绝。
- strong evidence 只有存在共同 source 时兼容。
- weak evidence 可在 normal-tensor 通道内兼容。
...
- weak barrier：normal-tensor；可用 `includeWeakEvidence=false` 排除。
...
`simplify` 暴露法线过滤和 consolidation，并在相应开关出现时启用 feature-curve policy；surface patch 只属于 feature-analysis 命令，不参与 QEM collapse 决策。
...
- curve network 与局部几何证据：M016、M024-M026、M044。
- CAD/STL graph continuity：M007、M016、M018、M019。
- weak feature consolidation：M026。
- engineering patch segmentation：M024、M025。

开源程序主要作为实现边界和测试 oracle，而不是复制 API：OpenMesh（halfedge/property/policy 分离）、CGAL PMP（显式 constrained edges）、pmp-library/libigl（局部邻域与属性组织）、MeshLib/L0Denoising/NLLR/LSD（法线域或 feature-preserving denoising 对照）。
