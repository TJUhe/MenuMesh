...
本计划按当前库状态更新。当前项目已经有稳定的 CMake/VS Code 构建、C++/C SDK、CLI、平级特征检测模块、特征曲线保护和 73 个非性能回归测试。下一阶段目标是把“可演示的 line quadrics 简化器”推进为“可嵌入的工业网格简化内核”。
...
当前项目不是完整 CAD 内核，也不是通用网格修复器。它现在最可靠的定位是：一个带 C++/C SDK 的三角网格 QEM/line-quadrics 简化库，支持特征检测、曲线保护和保守合法性过滤，适合继续向工业网格 decimation 模块演进。
