...
void readFeatureBenchmarkLabels(
    const fs::path& path, std::vector<std::pair<int, int>>& edges, std::vector<int>& junctions
) {
...
int commandSimplify(const Args& args) {
    ...
    if (!metricsCsv.empty()) {
        ...
        csv << manumesh::simplification::statsRowCsv("output", outStats, &distance) << "," << report.collapsedEdges
            ...
            << report.projectedFeaturePlacements << "," << manumesh::simplification::toString(report.terminationReason)
