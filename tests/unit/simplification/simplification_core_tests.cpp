...
TEST(ManuMesh, MeshQueriesComputeLocalVertexEdgeScale) {
    ...
    const std::vector<double> scale = manumesh::detail::computeVertexAverageEdgeLength(mesh);
...
TEST(ManuMesh, ComputesMeshStatsForGeneratedCube) {
    ...
    const manumesh::simplification::MeshStats stats = manumesh::simplification::computeMeshStats(input);
...
TEST(ManuMesh, MeshUtilitiesRejectMalformedInputWithoutThrowing) {
    const std::filesystem::path objPath = std::filesystem::temp_directory_path() / "line_quadrics_bad_face.obj";
    {
    ...
    invalid.faces = {{{0, 1, 2}}};
    error.clear();
    const std::filesystem::path stlPath = std::filesystem::temp_directory_path() / "line_quadrics_invalid.stl";
