...
- 回归测试：247 个启用的非性能 CTest（其中 `ctest -LE "performance|external"` 快速套件 236 个，external 大网格用例 11 个），另有 performance 构建路径。分层与命令见 [`testing_strategy.md`](testing_strategy.md)。
