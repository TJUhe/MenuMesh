...
bool curveBudgetAllows(const CollapseAttemptInput& input, const Vec3& position) {
  if (!input.options.preserveFeatureCurves ||
      input.options.maxFeatureCurveDeviationRatio <= 0.0) {
    return true;
  }
  ...
  const double maxDistance =
      input.options.maxFeatureCurveDeviationRatio * std::max(1e-12, input.meshDiagonal);
...
CollapseAttemptResult evaluateCollapseAttempt(const CollapseAttemptInput& input) {
  ...
  const FeatureCollapseRejectKind featureRejectKind =
      input.featurePolicy.collapseRejectKind(
          {input.edge, input.vertices, input.activeLoopCounts});
  ...
  const BoundaryCollapseDecision boundaryDecision = boundaryCollapseDecision(
      {input.edge, input.faces, input.vertices, input.topology, input.options});
  ...
  const bool featureCurveCollapse =
      input.featurePolicy.isHardProtectedCollapse(input.edge, input.vertices);
  ...
  const int placementCount =
      tryFallbackPlacements ? static_cast<int>(input.placements.size()) : 1;

  bool sawCurveBudgetReject = false;
  for (int placementIndex = 0; placementIndex < placementCount; ++placementIndex) {
    Vec3 collapsePosition = input.placements[placementIndex].position;
    projectBoundaryPlacement({input.edge, boundaryDecision, input.vertices},
                             collapsePosition);
    ...
    const bool projected = input.featurePolicy.projectPlacement(
        {input.edge, input.vertices, input.featureCurves}, collapsePosition);
    const CollapseRejectReason rejectReason =
        collapseRejectReason({input.edge,
                              collapsePosition,
