...
- `documentation/README.md` 的当前代码事实。
- `documentation/design/architecture.md` 的模块依赖图。
- `documentation/design/source_organization.md` 的目录契约。
- 必要时新增 `documentation/guide/<domain>_usage.md`。
- 若对 SDK 用户开放，新增或更新 `examples/` 和 `examples/sdk_consumer/`。

不要更新 `documentation/generated/notes/` 当作唯一说明；那里是导出笔记或历史材料。若内容受
