...
| `performance/` | 大模型、外部数据集和性能/压力测试源码。默认构建关闭，由 `LQ_BUILD_PERFORMANCE_TESTS=ON` 启用。 |
| `data/` | 纳入版本管理的 fixture、外部回归网格和数据集 case 列表。 |
| `output/` | CLI 验证命令生成的本地输出；被 git 忽略。 |

普通非性能测试仍然编译到一个 `line_quadrics_qem_tests` 可执行文件中，方便 VS Code task、CTest 标签和本地调试保持简单。性能测试单独编译到 `line_quadrics_qem_performance_tests`，并带 `performance` 标签。
