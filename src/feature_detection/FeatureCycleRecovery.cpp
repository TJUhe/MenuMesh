...
void recoverCircularCyclesThroughJunctions(const Mesh& mesh,
  ...
  for (int i = 0; i < static_cast<int>(chains.size()); ++i) {
    for (int j = i + 1; j < static_cast<int>(chains.size()); ++j) {
      ...
      addRecoveredCycle(RecoveredCycleKind::Circular, std::move(cycle), seenCycles,
  ...
  for (int seed = 0; seed < static_cast<int>(adjacency.size()); ++seed) {
    ...
    for (int v : component) {
      ...
      for (int nb : neighbors) {
        if (v >= nb ||
            treeEdges.find(manumesh::detail::meshEdgeKey(v, nb)) != treeEdges.end()) {
          continue;
        }
        std::vector<int> cycle = treePathCycle(v, nb, parent, depth);
        if (static_cast<int>(cycle.size()) <= kMaxCycleVertices) {
          addRecoveredCycle(RecoveredCycleKind::Polygonal, std::move(cycle), seenCycles,
                            mesh, options, trace, analysis, loopId);
        }
      }
    }
