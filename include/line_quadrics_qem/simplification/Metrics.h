...
LQ_API MeshStats computeMeshStats(const Mesh& mesh);
/// Estimates bidirectional mesh distance using deterministic vertex samples.
LQ_API DistanceStats compareMeshesBySampledDistance(const Mesh& original,
                                                    const Mesh& simplified,
                                                    int maxSamples);

/// CSV header for mesh statistics rows.
LQ_API std::string statsHeaderCsv();
