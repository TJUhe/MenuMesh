 ...
 * @brief 提供 STL、OBJ 网格读取以及 STL/OBJ 写出入口。
 * @ingroup manumesh_io
 *
...
/// 由于 STL 不携带 UV，成功时会清除 `mesh` 中已有的纹理坐标。失败时 `mesh` 保持不变。
...
/// @return 完整解析和校验成功时为 true；失败时 `mesh` 保持不变。
MANUMESH_API bool
...
/// 而不是生成无效三角形。顶点记录的可选第四个分量按齐次 `w` 处理；顶点法向（`vn`）只用于校验引用，
/// 未知指令会被忽略。失败时 `mesh` 保持不变。
/// @param[in] path UTF-8 输入文件路径。
...
/// @return 文件格式受支持且有效时为 true；失败时 `mesh` 保持不变。
MANUMESH_API bool
...
/// 返回 false 并设置 `error`。写入通过同目录临时文件完成，失败不会截断已有目标文件。
...
/**
 * @brief 将三角网格写入 OBJ，并保留有效的逐角纹理坐标。
 *
 * 写出使用临时文件替换目标，因此写入失败不会损坏已有目标文件。每个带纹理三角形写出三条
 * 独立的 `vt` 记录，以完整保留逐角 UV；没有 UV 的面写作纯位置索引。
 */
MANUMESH_API bool saveObj(const std::string& path, const Mesh& mesh, std::string* error = nullptr);
/// 将网格写为标准小端二进制 STL 文件。
///
/// 二进制 STL 将每个坐标存为 IEEE-754 float32，最多存储 UINT32_MAX 个三角形。不写入纹理坐标。
/// 当网格无效、坐标超出 float32 范围或文件无法创建/完整写入时返回 false；失败不会截断已有目标文件。
/// @param[in] path UTF-8 目标路径。
/// @param[in] mesh 严格有效的三角网格。
/// @param[out] error 可选的校验或 I/O 诊断信息。
