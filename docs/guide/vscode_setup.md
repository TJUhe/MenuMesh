...
本项目以 VS Code 任务、显式 CMake 命令和 CMake Tools 为主要工作流。旧的根目录 PowerShell 脚本已经移除；常用构建、测试、演示和验证流程都在 `.vscode/tasks.json` 中维护。
...
本项目支持 CMake 3.18.6 及以上版本，不依赖 `CMakePresets.json`。VS Code 任务直接调用 `cmake -S ... -B ...`，所以同样命令也可以在普通终端中运行。
