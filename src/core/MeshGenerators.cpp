  ...
  for (int y = 0; y <= n; ++y) {
    for (int x = 0; x <= n; ++x) {
      mesh.vertices.emplace_back((static_cast<double>(x) / n - 0.5) * size,
                                 (static_cast<double>(y) / n - 0.5) * size, 0.0);
    }
  }
...
Mesh generateBumpGrid(int n, double size) {
  ...
  for (int y = 0; y <= n; ++y) {
    for (int x = 0; x <= n; ++x) {
      ...
      const double cross =
          0.08 * std::exp(-180.0 * px * px) + 0.08 * std::exp(-180.0 * py * py);
...
Mesh generateSteppedShaftGrid(int n) {
  ...
  const std::vector<std::pair<double, double>> profile = {
      {0.32, -1.15}, {0.32, -0.72}, {0.48, -0.72}, {0.48, -0.20}, {0.36, -0.20},
      {0.36, 0.32},  {0.58, 0.32},  {0.58, 0.78},  {0.42, 0.78},  {0.42, 1.08},
  };
...
Mesh generatePipeCouplingGrid(int n) {
  ...
  const std::vector<std::pair<double, double>> profile = {
      {0.38, -1.00}, {0.72, -1.00}, {0.72, -0.42}, {0.58, -0.42}, {0.58, 0.42},
      {0.72, 0.42},  {0.72, 1.00},  {0.38, 1.00},  {0.38, -1.00},
  };
  return generateLatheProfile(profile, segments);
}
...
Mesh generatePulleyGrid(int n) {
  const int segments = std::max(72, n);
  const std::vector<std::pair<double, double>> profile = {
      {0.26, -0.58}, {0.62, -0.58}, {0.76, -0.26}, {0.60, 0.0},
      {0.76, 0.26},  {0.62, 0.58},  {0.26, 0.58},  {0.26, -0.58},
  };
  return generateLatheProfile(profile, segments);
}
